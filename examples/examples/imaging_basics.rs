...
use understory_imaging::{
    BlendMode as ImagingBlendMode, DrawOp, ImagingBackend, PaintDesc, PathCmd, PathDesc,
    ...
    B: ImagingBackend + ResourceBackend,
{
    ...
    let normal = ImagingBlendMode::default();
    let multiply = ImagingBlendMode::from(Mix::Multiply);
    let exclusion = ImagingBlendMode::from(Mix::Exclusion);

    // Row 1: SrcOver with varying opacity.
    let y1 = 110.0;
    backend.state(StateOp::SetTransform(Affine::translate((140.0, y1))));
    backend.state(StateOp::SetBlendMode(normal));
    ...
    backend.state(StateOp::SetBlendMode(multiply));
    backend.state(StateOp::SetPaint(red_paint));
    backend.draw(DrawOp::FillPath(left_circle));
    backend.state(StateOp::SetPaint(blue_paint));
    backend.draw(DrawOp::FillPath(right_circle));
    ...
    backend.state(StateOp::SetBlendMode(exclusion));
    backend.state(StateOp::SetPaint(red_paint));
    backend.draw(DrawOp::FillPath(left_circle));
    backend.state(StateOp::SetPaint(blue_paint));
    backend.draw(DrawOp::FillPath(right_circle));
    ...
    backend.state(StateOp::SetTransform(Affine::IDENTITY));
    backend.state(StateOp::SetClip(understory_imaging::ClipShape::Rect {
        x0: 60.0,
        y0: 40.0,
        x1: 260.0,
        y1: 220.0,
    ...
    backend.state(StateOp::SetPaint(red_paint));
    backend.draw(DrawOp::FillPath(big_circle));
    backend.state(StateOp::SetClip(understory_imaging::ClipShape::Infinite));
    ...
    let colr_subset_bytes: &[u8] =
        include_bytes!("../../../vello/examples/assets/noto_color_emoji/NotoColorEmoji-Subset.ttf");
    let emoji_text = "🎉🤠✅";
