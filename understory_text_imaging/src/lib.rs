...
use understory_imaging::{
    ClipShape, DrawOp, ImageDesc, ImagingBackend, ImagingOp, PaintDesc, PaintId, PathCmd, PathDesc,
    PathId, ResourceBackend, StateOp, TransformClass,
};
...
pub fn draw_glyph_run<B: ImagingBackend + ResourceBackend>(
    ...
    for inst in glyphs {
        ...
        if let Some(bitmap) = strikes.glyph_for_size(bitmap_size, gid) {
            ...
            let image = backend.create_image(ImageDesc { width, height }, &pixels);
...
struct ImagingColorPainter<'a, B> {
    ...
    current_clip: ClipShape,
    /// Number of active COLR layers (BeginGroup/EndGroup pairs).
    layer_depth: u32,
    /// Number of active COLR clips (glyph/box); nested clips are flattened
    /// into a single imaging clip but depth is tracked for defensive unwind.
    clip_depth: u32,
}
    ...
    B: ResourceBackend,
{
    fn new(
        ...
        Self {
            ...
            current_clip: ClipShape::Infinite,
            layer_depth: 0,
            clip_depth: 0,
        }
    }
    ...
    fn finalize(&mut self) {
        // Unwind any remaining groups by emitting balancing EndGroup ops.
        while self.layer_depth > 0 {
            self.layer_depth -= 1;
            self.ops.push(ImagingOp::State(StateOp::EndGroup));
        }

        // If we saw any clips and the current clip is still non-infinite,
        // restore an infinite clip to avoid leaking COLR clip state.
        if self.clip_depth > 0 && !matches!(self.current_clip, ClipShape::Infinite) {
            self.current_clip = ClipShape::Infinite;
            self.ops
                .push(ImagingOp::State(StateOp::SetClip(ClipShape::Infinite)));
        }
    ...
    B: ResourceBackend,
{
    ...
    fn push_clip_glyph(&mut self, glyph_id: GlyphId) {
        ...
        {
            self.current_clip = ClipShape::Path(path);
            ...
            self.ops
                .push(ImagingOp::State(StateOp::SetClip(ClipShape::Path(path))));
        }
    }
    ...
    fn push_clip_box(&mut self, clip_box: BoundingBox) {
        ...
        self.current_clip = ClipShape::Rect { x0, y0, x1, y1 };
        self.clip_depth = self.clip_depth.saturating_add(1);
        self.ops.push(ImagingOp::State(StateOp::SetTransform(
            self.current_transform,
        )));
        self.ops
            .push(ImagingOp::State(StateOp::SetClip(ClipShape::Rect {
                x0,
                y0,
                x1,
                y1,
    ...
    fn pop_clip(&mut self) {
        // For the initial implementation, popping a clip simply restores
        // an infinite clip region. Nested clips are not yet modeled.
    ...
    fn fill(&mut self, brush: ColrBrush<'_>) {
        ...
        let path = match self.current_clip {
            ClipShape::Path(id) => Some(id),
            ClipShape::Rect { x0, y0, x1, y1 } | ClipShape::RoundedRect { x0, y0, x1, y1, .. } => {
    ...
    fn push_layer(&mut self, composite_mode: ColrCompositeMode) {
        ...
        self.ops.push(ImagingOp::State(StateOp::BeginGroup {
            blend,
            opacity: 1.0,
        }));
    }
...
fn colr_glyph_to_picture<B>(
    ...
    font_size_px: f32,
    context_color: Color,
) -> Option<understory_imaging::PictureId>
where
    B: ResourceBackend,
{
    ...
    let picture = backend.create_picture(understory_imaging::PictureDesc {
...
mod tests {
    ...
    use understory_imaging::{DrawOp, ImagingOp, PaintDesc, PathDesc, ResourceBackend, StateOp};
    ...
    struct PictureCapturingBackend {
        ...
        next_paint: u32,
        next_picture: u32,
        last_picture: Option<understory_imaging::PictureDesc>,
    ...
    impl ResourceBackend for PictureCapturingBackend {
        ...
        fn destroy_image(&mut self, _id: understory_imaging::ImageId) {}
        ...
        fn create_picture(
            &mut self,
            desc: understory_imaging::PictureDesc,
        ) -> understory_imaging::PictureId {
            let id = self.next_picture;
            self.next_picture += 1;
            self.last_picture = Some(desc);
            understory_imaging::PictureId(id)
        }

        fn destroy_picture(&mut self, _id: understory_imaging::PictureId) {}
    }
    ...
    fn colr_subset_glyph_produces_ops() {
        ...
        let font_bytes: &[u8] = include_bytes!(
            "../../../vello/examples/assets/noto_color_emoji/NotoColorEmoji-Subset.ttf"
        );
        let font_ref = FontRef::new(font_bytes).expect("valid font");
    ...
    /// pictures do not leak `BeginGroup` without matching `EndGroup`.
    #[test]
    fn imaging_color_painter_unwinds_layers_and_clips() {
        ...
        // Finalize should emit balancing EndGroup/SetClip(Infinite) ops and
        // reset the internal depth counters.
        painter.finalize();

        // Expect at least one EndGroup and a SetClip(Infinite) at the tail.
        ...
        assert!(
            painter
                .ops
                .iter()
                .any(|op| matches!(op, ImagingOp::State(StateOp::EndGroup))),
            "expected at least one EndGroup emitted during finalize"
        );

        match painter.ops.last() {
            Some(ImagingOp::State(StateOp::SetClip(ClipShape::Infinite))) => {}
            other => panic!("expected final op to reset clip to Infinite, got {other:?}"),
        }
    }
    ...
    fn glyph_ids_run_uses_bitmaps_when_no_outlines() {
        ...
        let font_bytes: &[u8] = include_bytes!(
            "../../../vello/examples/assets/noto_color_emoji/NotoColorEmoji-CBTF-Subset.ttf"
        );
        let font_ref = FontRef::new(font_bytes).expect("valid font");
        let cmap = font_ref.charmap();
